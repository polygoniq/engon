...
def get_node_tree_inputs(node_tree: bpy.types.NodeTree) -> typing.Iterable[NodeSocketInterfaceCompat]:
    """Returns inputs of given 'node_tree' reassuring compatibility pre and post Blender 4.0"""
    assert isinstance(node_tree, bpy.types.NodeTree)
    if bpy.app.version < (4, 0, 0):
        yield from node_tree.inputs
    else:
        # In 4.0 the node_tree.inputs was moved to interface.items_tree with in_out == 'INPUT'
...
def get_channel_nodes_map(
    ...
    image_nodes = find_nodes_in_tree(
        node_tree,
        lambda x: isinstance(x, bpy.types.ShaderNodeTexImage)
    )

    channel_nodes_map: typing.DefaultDict[
        str, typing.List[bpy.types.ShaderNodeTexImage]] = collections.defaultdict(list)
...
def draw_node_tree(
    ...
    def draw_node_and_recurse(
        ...
        box = layout.box()
        row = box.row()
        row.prop(node, "hide", text="", emboss=False,
                 icon='TRIA_RIGHT' if node.hide else 'TRIA_DOWN')
    ...
    material_output_nodes = find_nodes_in_tree(
        node_tree,
        lambda x: isinstance(x, bpy.types.ShaderNodeOutputMaterial)
    )
